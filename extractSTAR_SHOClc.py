...
#   COMPlightcurve = open(file+'_compsLC_star_'+str(starnumber),'w')
   ...
   try:
      ...
      extdatalist = numpy.loadtxt(file,dtype="str",usecols=(7,8),unpack=True)
      hjdyes = 0
...
#          print >> RAWlightcurve, datalist[0][i], datalist[1][i], datalist[2][i], hjdvalue, datalist[4][i], datalist[5][i], "#"  
          if hjdyes==1:
             ...
             target.append((hjdvalue, datalist[4][i], datalist[5][i], datalist[0][i], datalist[1][i], datalist[2][i], datalist[6][i], extdatalist[0][i], extdatalist[1][i]))
      else:
          if hjdyes==1:
             ...
             comparisons.append((int(datalist[0][i]), hjdvalue, datalist[4][i], datalist[5][i], datalist[0][i], datalist[1][i], datalist[2][i], datalist[6][i], extdatalist[0][i], extdatalist[1][i]))
   ...
   for j in range(len(target)):
       ...
       if n > 0:
           if hjdyes==1:
               ...
               diffphot.append([target[j][0], target[j][1]/(allcomparisons/n),target[j][2],target[j][3],target[j][4],target[j][5],target[j][6],target[j][7],target[j][8]])
   ...
   for k in range(len(diffphot)):
       if hjdyes==1:
           ...
           print >> differential, int(float(diffphot[k][3])), diffphot[k][4], diffphot[k][5], round(float(diffphot[k][0]),6), diffphot[k][1]*overallaverage, diffphot[k][2], diffphot[k][6], diffphot[k][7], diffphot[k][8]
